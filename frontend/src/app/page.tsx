...
const navItems = ["Platform", "Workforce", "Intelligence"];

const departments = [
  { name: "Finance", detail: "Margin, cash, and variance signals in one operating view.", icon: Landmark },
  { name: "Revenue", detail: "Priority accounts, pipeline risk, and next best action.", icon: UsersRound },
  { name: "Operations", detail: "Early warnings before friction becomes failure.", icon: Factory },
  { name: "Knowledge", detail: "Trusted institutional memory, available at decision speed.", icon: FileSearch },
];

const intelligence = [
  "Executive decisions connected to operational context.",
  "Institutional memory that understands teams, files, and permissions.",
  "A quiet system of intelligence, built for daily command.",
];

const dashboardRows = [
  ["Revenue growth", "+18.4%", "Stable", "text-teal-200"],
  ["Operating cost", "-7.2%", "Under review", "text-gold"],
  ["Response time", "2.8m", "On target", "text-teal-200"],
  ["Data readiness", "94%", "Trusted", "text-white"],
];
        ...
        <div className="hidden items-center gap-1 lg:flex">
          {navItems.map((item) => (
            <a
              key={item}
              href={`#${item}`}
              className="rounded-md px-3 py-1.5 text-xs font-medium text-white/48 transition hover:bg-white/[0.045] hover:text-white/82"
            >
              {item}
            </a>
          ))}
  ...
  return (
    <section id="Platform" className="mx-auto grid min-h-[calc(100vh-61px)] max-w-7xl items-center gap-16 px-5 py-24 sm:px-6 lg:grid-cols-[0.82fr_1fr] lg:px-8 lg:py-28">
      <motion.div initial="hidden" animate="visible" variants={fadeUp} transition={{ duration: 0.7, ease: "easeOut" }} className="max-w-xl">
        <div className="text-sm font-semibold text-white/64">NAWA</div>
        ...
        <div className="mt-14 grid max-w-md grid-cols-3 gap-5 border-t border-white/[0.07] pt-6">
          {["Decision speed", "Operational memory", "Governed access"].map((item) => (
      ...
      <div className="relative overflow-hidden rounded-md border border-white/[0.08] bg-[#09121f]/88 shadow-[0_28px_80px_rgba(0,0,0,0.34)]">
        ...
        <div className="grid gap-4 p-4 lg:grid-cols-[190px_1fr]">
          <aside className="space-y-2.5 border-r border-white/[0.07] pr-4">
            {["Command", "Finance", "Revenue", "Operations"].map((item, index) => (
            ...
            <div className="rounded-md bg-gold/[0.075] p-3">
              <div className="text-xs font-semibold text-gold/90">Signal</div>
          ...
          <div className="space-y-4">
            <div className="grid gap-3 sm:grid-cols-3">
              {[
                ["Health", "87%", Gauge],
                ["Decisions", "12", BrainCircuit],
                ["Sources", "348", Database],
                ...
                <div key={label as string} className="rounded-md bg-white/[0.035] p-4">
                  ...
                  <div className="mt-1 text-xs font-medium text-white/36">{label as string}</div>
                </div>
              ))}
            </div>
            ...
            <div className="grid gap-3 lg:grid-cols-[1fr_210px]">
              <div className="rounded-md bg-white/[0.03] p-4">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-semibold text-white/88">Operating pulse</span>
              ...
              <div className="rounded-md bg-white/[0.03] p-4">
                <div className="text-sm font-semibold text-white/88">Decision path</div>
                <div className="mt-5 space-y-3">
                  {["Context", "Risk", "Action"].map((item) => (
            ...
            <div className="rounded-md bg-[#07101b]/88">
              {dashboardRows.map(([metric, value, status, tone]) => (
                <div key={metric} className="grid grid-cols-3 gap-3 border-b border-white/[0.055] px-4 py-3 last:border-b-0">
                  <span className="text-xs font-medium text-white/48">{metric}</span>
...
function DepartmentsSection() {
  return (
    <section id="Workforce" className="mx-auto max-w-7xl px-4 py-24 sm:px-6 lg:px-8">
      <SectionIntro eyebrow="AI Workforce" title="Specialized intelligence for every function." text="Each team gets a focused operating layer tuned to its work, data, and decisions." />
      <div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {departments.map(({ name, detail, icon: Icon }, index) => (
          <motion.div key={name} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.3 }} variants={fadeUp} transition={{ duration: 0.5, delay: index * 0.06 }}>
            <Card className="h-full transition duration-300 hover:-translate-y-1 hover:border-teal-200/25 hover:bg-white/[0.075]">
              <CardHeader className="p-6">
                ...
                <CardTitle className="mt-6 text-lg">{name}</CardTitle>
              </CardHeader>
  ...
  return (
    <section id="Intelligence" className="border-y border-white/10 bg-white/[0.025]">
      <div className="mx-auto grid max-w-7xl gap-14 px-4 py-24 sm:px-6 lg:grid-cols-[0.95fr_1.05fr] lg:px-8">
        <SectionIntro eyebrow="Operational Intelligence" title="From scattered data to institutional command." text="NAWA reads signals, understands constraints, and moves decisions into action." />
        <div className="space-y-4">
          {intelligence.map((item, index) => (
            <motion.div key={item} initial={{ opacity: 0, x: -18 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.45, delay: index * 0.08 }} className="rounded-md border border-white/10 bg-[#09121f]/80 p-6">
...
function DashboardShowcase() {
  return (
    <section id="Dashboard" className="mx-auto max-w-7xl px-4 py-24 sm:px-6 lg:px-8">
      <SectionIntro eyebrow="Dashboard Showcase" title="Information density without congestion." text="A command interface for daily operating rhythm, built with precision and restraint." />
      <div className="mt-12 grid gap-5 lg:grid-cols-[1fr_360px]">
        <div className="rounded-md border border-white/10 bg-[#09121f]/88 p-6">
          <div className="grid gap-4 md:grid-cols-3">
            {["Critical requests", "Contract risk", "Growth signals"].map((item, index) => (
          ...
          <div className="mt-5 rounded-md border border-white/10 bg-[#070d16] p-5">
            <div className="mb-6 flex items-center justify-between">
              <span className="text-sm font-semibold">Decision chain</span>
              <ShieldCheck className="h-4 w-4 text-gold" />
            </div>
            <div className="grid gap-3 md:grid-cols-4">
              {["Source", "Verify", "Infer", "Execute"].map((step) => (
        ...
        <Card>
          ...
          <CardContent className="space-y-3 px-6 pb-6">
            {["Company files", "Team memory", "User permissions", "Auditable output"].map((item) => (
  ...
  return (
    <section id="Architecture" className="mx-auto max-w-7xl px-4 py-24 sm:px-6 lg:px-8">
      <div className="grid gap-10 rounded-md border border-white/10 bg-white/[0.035] p-7 sm:p-10 lg:grid-cols-[0.8fr_1.2fr]">
        <div>
          <div className="text-xs font-semibold uppercase text-gold">Enterprise Architecture</div>
          <h2 className="mt-5 text-4xl font-semibold leading-[1.14] sm:text-5xl">Intelligence as infrastructure.</h2>
        </div>
        <div className="grid gap-4 md:grid-cols-3">
          {[
            ["Native workflow", "Built into how decisions move."],
            ["Institutional context", "Understands the operating model."],
            ["Governed access", "Trust inside clear boundaries."],
          ].map(([title, text]) => (
